...
describe("POST /auth/login", function() {
  ...
  //TESTS BUG #1 - add test: incorrect username/password given, should raise 401.
  test("should raise 401 if incorrect username/password", async function() {
    const response = await request(app)
    .post("/auth/login")
    .send({
      username: "u1",
    ...
    //response.body returns {token: 'xxxx'}
    
    expect(response.statusCode).toBe(401);
    expect(response.body.message).toEqual('Cannot authenticate');
  });
});
...
describe("PATCH /users/[username]", function() {
  ...
  //TESTS BUG #4
  test("should patch data if user themselves & patch first_name, last_name, phone & email 4 fields", async function() {
    const response = await request(app)
      .patch("/users/u2")
...
describe("DELETE /users/[username]", function() {
  ...
  //TESTS BUG #5
  test("return 404 if user cannot be found", async function() {
    const response = await request(app)
      .delete("/users/u5")
      .send({ _token: tokens.u3 }); // u3 is admin
    expect(response.statusCode).toBe(404);
